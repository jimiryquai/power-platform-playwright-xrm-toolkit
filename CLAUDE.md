...
```bash
# 1. Clone and install
git clone https://github.com/jimiryquai/power-platform-playwright-xrm-toolkit.git
cd power-platform-playwright-xrm-toolkit
npm install
npm run build
...
# Browser used for the sign-in flow. Unset = msedge (playwright-ms-auth's default).
# Set to 'chromium' if managed-Edge policy kills the automation browser mid-redirect
# — see Troubleshooting. Password credentials only; certificate auth always uses msedge.
# MS_AUTH_BROWSER_CHANNEL=chromium
...
| `npm run auth` dies with `Target page, context or browser has been closed` | Managed-Edge policy (automatic profile switching / forced sign-in) terminates the automation browser during the auth redirect | Set `MS_AUTH_BROWSER_CHANNEL=chromium` in `.env` — the tests already run on bundled Chromium |

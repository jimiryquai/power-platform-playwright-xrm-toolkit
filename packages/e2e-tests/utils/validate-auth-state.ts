...
/**
 * Does a cookie domain belong to a Dataverse/CRM organisation?
 *
 * Organisations live on a per-region host — `crm.dynamics.com` (North America),
 * `crm4.dynamics.com` (Europe), `crm11.dynamics.com` (UK), and so on — so a
 * literal `crm.dynamics.com` match only recognises North American orgs.
 */
export function isDataverseDomain(domain: string): boolean {
  return /(^|\.)crm\d*\.dynamics\.com$/.test(domain);
}
...
export function validateAuthState(): AuthValidationResult {
  ...
  // We don't check individual cookie expiry — session cookies on the Dataverse
  // domain typically last longer than a day, and the file-age check below is the
  // authority.
  if (projectType === 'mda') {
    try {
      const stateData = JSON.parse(readFileSync(storageStatePath, 'utf-8'));
      const crmCookies = (stateData.cookies || []).filter(
        (c: any) => c.domain && isDataverseDomain(c.domain)
      );
  ...
  // A non-numeric override would yield NaN, and every `ageMs > NaN` comparison is
  // false — silently disabling the staleness check rather than tightening it.
  const configuredMaxAge = parseInt(process.env.MS_AUTH_STORAGE_STATE_EXPIRATION || '24', 10);
  const maxAgeHours = Number.isFinite(configuredMaxAge) ? configuredMaxAge : 24;
